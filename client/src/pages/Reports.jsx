...
const Reports = () => {
    ...
    return (
        <div className="space-y-4 pb-24 lg:pb-8">
            ...
            <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-200 print:hidden">
                <div className="flex flex-wrap gap-2">
                    {navButtons.map(btn => (
                        <button
                            key={btn.id}
                            onClick={() => { setReportType(btn.id); setReportData([]); }}
                            className={`flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold rounded-lg transition-all flex-grow sm:flex-grow-0 ${reportType === btn.id ? 'bg-primary-50 text-primary-700 shadow-sm ring-1 ring-primary-200' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 bg-white border border-gray-100'}`}
                        >
                            {btn.label}
                        </button>
                    ))}
            ...
            {selectedUser && (
                <Drawer 
                    ...
                    title={`Documents Generated by ${selectedUser.name}`}
                    description={`Invoices: ${selectedUser.invoiceCount} (${formatCurrency(selectedUser.invoiceAmount)}) | Quotations: ${selectedUser.quotationCount} (${formatCurrency(selectedUser.quotationAmount)})`}
                    size="xl"
                >
                    <div className="overflow-x-auto w-full">
                        <table className="min-w-full">
                            <thead className="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">Date</th>
                                    <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">Document #</th>
                                    <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">Customer</th>
                                    <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">Type / Status</th>
                                    <th className="px-3 py-2 sm:px-4 sm:py-3 text-right text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">Amount</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {selectedUser.orders?.map(order => (
                                    <tr key={order._id} className="hover:bg-gray-50/50">
                                        <td className="px-3 py-2 sm:px-4 sm:py-3 whitespace-nowrap text-[11px] sm:text-xs text-gray-600 font-medium">{new Date(order.orderDate).toLocaleDateString()}</td>
                                        <td className="px-3 py-2 sm:px-4 sm:py-3 whitespace-nowrap text-[11px] sm:text-xs font-bold text-indigo-600">{order.orderNumber}</td>
                                        <td className="px-3 py-2 sm:px-4 sm:py-3 whitespace-nowrap text-[11px] sm:text-xs font-bold text-gray-800">{order.customer?.companyName || order.customer?.name || 'Unknown'}</td>
                                        <td className="px-3 py-2 sm:px-4 sm:py-3 whitespace-nowrap text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-gray-500">
                                        ...
                                        <td className="px-3 py-2 sm:px-4 sm:py-3 whitespace-nowrap text-[11px] sm:text-xs font-black text-gray-900 text-right">{formatCurrency(order.totalAmount)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
