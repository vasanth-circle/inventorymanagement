...
const Finishes = () => {
    ...
    const handleOpenModal = (finish = null) => {
        if (finish) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (editingFinish) {
            const result = await editFinish(editingFinish._id, formData);
            if (result.success) { handleCloseModal(); toast.success('Finish updated successfully'); }
        } else {
            const result = await addFinish(formData);
            if (result.success) { handleCloseModal(); toast.success('Finish created successfully'); }
        }
    };

    const handleDelete = async (id) => {
        await confirmDelete('Are you sure you want to delete this finish?', async () => {
            await removeFinish(id);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3 items-center justify-between">
                <h1 className="text-2xl font-bold text-gray-900">Finish Management</h1>
                <button
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <table className="min-w-full divide-y divide-gray-100">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Finish Name</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-50">
                        {loading ? (
                            ...
                            filtered.map((finish, idx) => (
                                <tr key={finish._id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-4 py-2 text-xs text-gray-400 w-8">{idx + 1}</td>
                                    <td className="px-4 py-2 text-sm font-semibold text-gray-800">{finish.name}</td>
                                    <td className="px-4 py-2 text-xs text-gray-500 max-w-xs truncate">
                                        {finish.description || <span className="italic text-gray-300">—</span>}
                                    </td>
                                    <td className="px-4 py-2 text-right">
                                        <button onClick={() => handleOpenModal(finish)} className="text-xs text-primary-600 hover:text-primary-800 font-semibold mr-3">Edit</button>
                                        <button onClick={() => handleDelete(finish._id)} className="text-xs text-red-500 hover:text-red-700 font-semibold">Delete</button>
                                    </td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan="4" className="px-4 py-6 text-center text-sm text-gray-400">
                                    {search ? 'No finishes match your search.' : 'No finishes found. Add one to get started.'}
                                </td>
                            </tr>
                        )}
                    </tbody>
                ...
                {filtered.length > 0 && (
                    <div className="px-4 py-2 border-t border-gray-100 bg-gray-50 text-xs text-gray-400">
                        Showing {filtered.length} of {finishes.length} finishes
                    </div>
                )}
            </div>
