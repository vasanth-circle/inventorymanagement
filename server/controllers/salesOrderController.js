...
export const validatePartyCreditLock = async (req, res, partyId, newInvoiceAmount) => {
    ...
    const partyDoc = await Party.findOne({ _id: partyId, ...tenantQuery(req) });
    if (!partyDoc) return true; // Passed (or error handled elsewhere)

    let isManuallyUnlocked = false;
    if (partyDoc.unlockedUntil && new Date(partyDoc.unlockedUntil) > new Date()) {
    ...
    if (!isManuallyUnlocked) {
        ...
        const currentBalance = partyDoc.currentBalance || 0;

        // 1. Check Credit Limit
        if (creditLimit > 0 && (currentBalance + newInvoiceAmount) > creditLimit) {
            sendError(res, 400, `Cannot create bill: Party Credit Limit (₹${creditLimit}) exceeded. Pending balance: ₹${currentBalance}. New Bill: ₹${newInvoiceAmount}. Contact Admin to unlock.`);
            return false;
        }

        // 2. Check Credit Days (FIFO calculation)
        if (creditDays > 0 && currentBalance > 0) {
            const allLedgerEntries = await Ledger.find({ partyType: 'Party', party: partyId, tenantId: req.tenantId }).sort({ date: 1, createdAt: 1 });
            
            let totalPayments = 0;
            const bills = [];

            // Account for opening balance in FIFO calculation
            if (partyDoc.openingBalance > 0) {
                bills.push({ date: partyDoc.createdAt || new Date(0), amount: partyDoc.openingBalance });
            } else if (partyDoc.openingBalance < 0) {
                totalPayments += Math.abs(partyDoc.openingBalance);
            }
            ...
            if (oldestUnpaidBillDate) {
                ...
                if (daysPending > creditDays) {
                    sendError(res, 400, `Cannot create bill: Party has an unpaid balance pending for ${daysPending} days (Limit: ${creditDays} days). Contact Admin to unlock.`);
                    return false;
                }
            }
        }
...
// Recalculate party running ledger balance chronologically
export const recalculatePartyBalance = async (partyId, tenantId) => {
    try {
        const party = await Party.findById(partyId);
        if (!party) return;

        // Fetch all ledger entries sorted chronologically
        const entries = await Ledger.find({ partyType: 'Party', party: partyId, tenantId })
        ...
        await Party.findByIdAndUpdate(partyId, { currentBalance: runningBalance });
    } catch (err) {
        console.error(`recalculatePartyBalance error for party ${partyId}:`, err.message);
    }
};
...
export const syncSalesOrderLedger = async (orderId, tenantId, userId) => {
    try {
        ...
        const existingBillEntry = await Ledger.findOne({ partyType: 'Party', refId: orderId, refType: 'SalesOrder', type: 'bill' });
        const existingPaymentEntry = await Ledger.findOne({ partyType: 'Party', refId: orderId, refType: 'SalesOrder', type: 'payment' });

        if (isRealBill) {
            let oldParty = null;
            const newParty = order.party.toString();
            ...
            if (existingBillEntry) {
                oldParty = existingBillEntry.party.toString();
                
                existingBillEntry.party = order.party;
            ...
            if (order.advanceAmount > 0) {
                if (existingPaymentEntry) {
                    if (existingPaymentEntry.party.toString() !== newParty) {
                        oldParty = existingPaymentEntry.party.toString();
                    }
                    
                    existingPaymentEntry.party = order.party;
                ...
                if (existingPaymentEntry) {
                    ...
                    await Ledger.deleteOne({ _id: existingPaymentEntry._id });
                }
            }

            // ─── 3. Recalculate Balances ───
            await recalculatePartyBalance(newParty, tenantId);
            if (oldParty && oldParty !== newParty) {
                await recalculatePartyBalance(oldParty, tenantId);
            }
        } else {
            // If it shouldn't have ledger entries (e.g. toggled back to estimation or cancelled)
            const affectedPartys = new Set();
            if (existingBillEntry) {
                affectedPartys.add(existingBillEntry.party.toString());
                await Ledger.deleteOne({ _id: existingBillEntry._id });
            }
            if (existingPaymentEntry) {
                affectedPartys.add(existingPaymentEntry.party.toString());
                await Ledger.deleteOne({ _id: existingPaymentEntry._id });
            }

            for (const custId of affectedPartys) {
                await recalculatePartyBalance(custId, tenantId);
            }
        }
    } catch (err) {
...
export const getSalesOrders = async (req, res, next) => {
    try {
        const { status = '', party = '', type = '', search = '', page = 1, limit = 10, startDate, endDate, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
        ...
        if (search) {
            const matchingPartys = await Party.find({ 
                name: { $regex: search, $options: 'i' },
                ...tenantQuery(req)
            }).select('_id');
            const partyIds = matchingPartys.map(c => c._id);
        ...
        const orders = await SalesOrder.find(query)
            .populate('party', 'name companyName phone gstin address')
...
export const createSalesOrder = async (req, res, next) => {
    try {
        const { 
            party, items, orderDate, expectedShipmentDate, 
        ...
        // ── Duplicate Submission Guard (same user + same party + same items fingerprint within 15s) ──
        if (!isEstimation) {
        ...
        if (!isEstimation) {
            ...
            const passed = await validatePartyCreditLock(req, res, party, newInvoiceAmount);
        ...
        while (!order && retries < 10) {
            ...
            try {
                order = await SalesOrder.create({
                    ...
                    partyType: partyType || 'Regular Party',
...
export const updateSOStatus = async (req, res, next) => {
    try {
        ...
        if (order.isEstimation && ['confirmed', 'dispatched', 'partially_dispatched'].includes(status)) {
            ...
            // ── Party Credit & Lock Validation when converting to Invoice ──
            const passed = await validatePartyCreditLock(req, res, order.party, order.totalAmount);
            if (!passed) return; // Error already sent inside helper
...
export const updateSalesOrder = async (req, res, next) => {
    try {
        ...
        if (partyType !== undefined) order.partyType = partyType;
...
export const deleteSalesOrder = async (req, res, next) => {
    try {
        ...
        // 2. Remove Party Ledger Entries
        if (order.party) {
            await Ledger.deleteMany({ partyType: 'Party', refId: order._id, tenantId });
            await recalculatePartyBalance(order.party, tenantId);
        }
